...
func BenchmarkOneEmptyRoute(B *testing.B) {
	SetMode(TestMode)
	router := New()
...
func BenchmarkHello(B *testing.B) {
	SetMode(TestMode)
	router := New()
	router.GET("/ping", func(c *Context) { c.Writer.WriteString("hello") })
	runRequest(B, router, "GET", "/ping")
}

func BenchmarkHelloRender(B *testing.B) {
	SetMode(TestMode)
	router := New()
	router.GET("/ping", func(c *Context) { c.Reply(200, "hello") })
	runRequest(B, router, "GET", "/ping")
}

func BenchmarkContextInvoke(B *testing.B) {
	f := func(ctx *Context) {
		ctx.Invoke(func() string { return "hello" })
	}
	ctx := &Context{}
	B.ReportAllocs()
	B.ResetTimer()
...
func BenchmarkHelloHandlerFunc(B *testing.B) {
	SetMode(TestMode)
	router := New()
	router.GET("/ping", H(func() string { return "hello" }))
	runRequest(B, router, "GET", "/ping")
}
...
func BenchmarkOneRouteString(B *testing.B) {
	...
	router.GET("/text", func(c *Context) {
		c.Reply(200, "this is a plain text")
	})
...
func runRequest(B *testing.B, r http.Handler, method, path string) {
	// create fake request
...
func githubConfigRouter(s *Server) {
	for _, route := range githubAPI {
		s.Handle(route.method, route.path, func(c *Context) {
...
func exampleFromPath(path string) (string, tree.Params) {
	output := new(bytes.Buffer)
	params := make(tree.Params, 0, 6)
	start := -1
	for i, c := range path {
		if c == ':' {
			start = i + 1
		...
		if start >= 0 {
			if c == '/' {
				...
				params = append(params, tree.Param{
					Key:   path[start:i],
					Value: value,
				})
