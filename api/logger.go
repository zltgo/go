...
// Logger instances a Logger middleware that will write the logs to Api.DefaultWriter.
// By default Api.DefaultWriter = os.Stdout.
func Logger() Handler {
...
func LoggerWithWriter(out io.Writer, notlogged ...string) Handler {
	isTerm := true
	...
	return func(c *Context) {
		...
		if _, ok := skip[path]; !ok {
			...
			comment := errorsString(c.Errors)

			if raw != "" {
				path = path + "?" + raw
			}

			fmt.Fprintf(out, "[Api] %v |%s %3d %s| %13v | %15s |%s %-7s %s %s\n%s",
